...
function parseDate(query, options) {
    ...
    if (query.period) {
        if (query.date_end && isDate(query.date_end)) {
            result.push(
                processQuery(options.default.date_field, 'lt:'.concat(query.date_end)),
                processQuery(options.default.date_field, 'gte:'.concat(getDateStart(query.period, new Date(query.date_end)))))
        } else {
            result.push(
                processQuery(options.default.date_field, 'lt:'.concat(getDateStart('0d', new Date()))),
                processQuery(options.default.date_field, 'gte:'.concat(getDateStart(query.period, new Date()))))
        }
        return result
    }
    ...
    if (query.date_start && isDate(query.date_start)) {
        result.push(
            processQuery(options.default.date_field, 'gte:'.concat(query.date_start)))
        if (query.date_end && isDate(query.date_end)) {
            result.push(
                processQuery(options.default.date_field, 'lt:'.concat(query.date_end)))
        } else {
            result.push(options.default.date_field, '$lt:'.concat(new Date().toISOString()))
        }
    }
    return result
}
